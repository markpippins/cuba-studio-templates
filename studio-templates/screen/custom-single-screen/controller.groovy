...
import javax.inject.Named<%}%><%if (entity.genericIdType == 'IdProxy') {%>
...
class ${controllerName} extends AbstractLookup {
     ...
     */
    @Inject
    private CollectionDatasource<${entity.className}, ${entity.genericIdType}> ${tableDs}
    ...
    <%if (collectionAttributesTablesXml) {%>@Inject
    ...
    public void init(Map<String, Object> params) {
         ...
         */
        ${tableDs}.addItemChangeListener({def e ->
            if (e.getItem() != null) {
         ...
         */
        ${tableId}Remove.setAfterRemoveHandler({def removedItems -> ${editDs}.setItem(null)})
        <%}%>
        disableEditControls()
    }
    ...
    private void initEditComponents(boolean enabled) {
        <%if (collectionAttributesTablesXml) {%>
            ComponentsHelper.walkComponents(editBox, new ComponentVisitor() {
            @Override
                void visit(Component component, String name) {
                    if (component instanceof FieldGroup) {
                        ((FieldGroup) component).setEditable(enabled)
                    } else if (component instanceof Table) {
                        ((Table) component).setEnabled(enabled)
    //                    ((Table) component).getActions().stream().map({action -> action.setEnabled(enabled)})
                    }
                }
            })
